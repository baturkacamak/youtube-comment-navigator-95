...
async function run() {
  ...
  try {
    ...
    console.error('Failed to reach the edit page:', error);
    await context.close();
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      'Could not click "Store listing" tab explicitly, assuming we are already there or UI differs.',
      error
    );
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(
        '   [User Action Needed] I cannot robustly switch languages automatically without DOM access.'
      );
