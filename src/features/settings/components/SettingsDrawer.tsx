...
const SettingsDrawer: React.FC<SettingsDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      <div
        className={`relative inset-y-0 left-0 flex transition-all h-fit duration-500 pointer-events-none ${
          drawerClass
        }`}
      >
        <div
          className="bg-white dark:bg-gray-800 w-80 h-full shadow-lg p-4 flex flex-col justify-between pointer-events-auto relative z-50"
          role="dialog"
      ...
      {showNotification && (
        <NotificationBubble message={t('Settings saved!')} position="bottom-left" />
      )}
    </>
  );
};
