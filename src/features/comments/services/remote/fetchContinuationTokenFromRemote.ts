...
const extractSectionToken = (data: any): string | undefined => {
    try {
        const contents = safeGet(() => data?.contents?.twoColumnWatchNextResults?.results?.results?.contents) || [];
        
        // Strategy 1: Look for continuationItemRenderer in itemSectionRenderer
        let token = contents
            .map((content: any) => safeGet(() => content.itemSectionRenderer?.contents?.[0]?.continuationItemRenderer?.continuationEndpoint?.continuationCommand?.token))
...
const extractCommentsToken = (data: any, sortOrderIndex: number = 0): string | undefined => {
    try {
        ...
        for (const endpoint of endpoints) {
             const reloadContinuationItems = safeGet(() => endpoint.reloadContinuationItemsCommand?.continuationItems);
             const appendContinuationItems = safeGet(() => endpoint.appendContinuationItemsAction?.continuationItems);
             ...
             for (const item of items) {
                 ...
                 const subMenuItems = safeGet(() => item.commentsHeaderRenderer?.sortMenu?.sortFilterSubMenuRenderer?.subMenuItems);
                 if (subMenuItems && subMenuItems[sortOrderIndex]) {
                     token = safeGet(() => subMenuItems[sortOrderIndex].serviceEndpoint?.continuationCommand?.token);
        ...
        const contents = safeGet(() => data?.contents?.twoColumnWatchNextResults?.results?.results?.contents) || [];
        token = contents
            .map((content: any) => safeGet(() => content.itemSectionRenderer?.header?.[0]?.commentsHeaderRenderer?.sortMenu?.sortFilterSubMenuRenderer?.subMenuItems?.[sortOrderIndex]?.serviceEndpoint?.continuationCommand?.token))
            .find((t: string | undefined) => t);
...
export const getContinuationTokenFromData = (data: any, isFetchingReply: boolean = false): string | null => {
...
export const fetchContinuationTokenFromRemote = async (videoId?: string): Promise<string> => {
    try {
        ...
        if (!sectionToken) {
            logger.warn('[fetchContinuationTokenFromRemote] Could not find Section Token in Step 1. Comments might be disabled or layout changed.');
            return '';
        }

        logger.info(`[fetchContinuationTokenFromRemote] Found Section Token: ${sectionToken.substring(0, 20)}...`);
        ...
        logger.info(`[fetchContinuationTokenFromRemote] Successfully retrieved Final Token: ${finalToken.substring(0, 20)}...`);
        return finalToken;
