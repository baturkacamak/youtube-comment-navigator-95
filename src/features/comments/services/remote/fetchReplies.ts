...
export const fetchRepliesJsonDataFromRemote = async (
  rawJsonData: any,
  windowObj: any,
  signal: AbortSignal
): Promise<any[]> => {
  ...
  const processToken = async (token: string): Promise<string[]> => {
    ...
    const continuationItems =
      replyData.onResponseReceivedEndpoints?.[0]?.appendContinuationItemsAction
        ?.continuationItems ||
      replyData.onResponseReceivedEndpoints?.[1]?.reloadContinuationItemsCommand
        ?.continuationItems ||
      [];
  ...
  const continuationItems =
    rawJsonData.onResponseReceivedEndpoints?.[0]?.appendContinuationItemsAction
      ?.continuationItems ||
    rawJsonData.onResponseReceivedEndpoints?.[1]?.reloadContinuationItemsCommand
      ?.continuationItems ||
    [];
