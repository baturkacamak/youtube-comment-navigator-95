export const extractContinuationToken = (continuationItems: any[]): string | null => {
  ...
  return (
    lastItem?.continuationItemRenderer?.continuationEndpoint?.continuationCommand?.token || null
  );
};
...
export const extractReplyContinuationTokens = (continuationItems: ContinuationItem[]): string[] => {
  ...
  for (let index = 0; index < continuationItems.length; index++) {
    ...
    const token =
      continuationItem.commentThreadRenderer?.replies?.commentRepliesRenderer?.contents?.[0]
        ?.continuationItemRenderer?.continuationEndpoint?.continuationCommand?.token ||
      continuationItem.commentThreadRenderer?.replies?.commentRepliesRenderer?.continuations?.[0]
        ?.nextContinuationData?.continuation ||
      continuationItem?.continuationItemRenderer?.button?.buttonRenderer?.command
