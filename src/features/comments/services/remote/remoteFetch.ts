...
export const fetchCommentsFromRemote = async (dispatch: any, bypassCache: boolean = false) => {
  ...
  try {
    ...
    const localToken = bypassCache ? null : await getContinuationToken(videoId);
    ...
    const token: string | null = localToken || (await fetchContinuationTokenFromRemote());
...
async function deleteCommentsIfFreshFetch(
