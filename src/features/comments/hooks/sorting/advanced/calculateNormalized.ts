...
const calculateNormalized = (comment: Comment, maxValues: MaxValues) => {
  ...
  const normalizedWordCount =
    maxValues.wordCount > 0 ? (comment.wordCount ?? 0) / maxValues.wordCount : 0;

  return (
    normalizedLikes * LIKE_WEIGHT +
    normalizedReplies * REPLY_WEIGHT +
    normalizedWordCount * WORD_WEIGHT
  );
};
