...
import { linkifyTimestampsInSafeHtml } from '../../shared/utils/timestamps';

interface TimestampedMarkdownProps {
  text: string;
}

const handleGeneratedTimestampClick = (event: MouseEvent, container: HTMLDivElement): void => {
  if (!(event.target instanceof Element)) return;

  const button = event.target.closest<HTMLButtonElement>('button[data-timestamp]');
...
const TimestampedMarkdown: React.FC<TimestampedMarkdownProps> = ({ text }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const html = linkifyTimestampsInSafeHtml(markdownToSafeHtml(text));

  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    const handleClick = (event: MouseEvent) => handleGeneratedTimestampClick(event, container);
...
export default TimestampedMarkdown;
