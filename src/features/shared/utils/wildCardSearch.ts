 ...
 */
export const wildCardSearch = (path: WildCardSearchPath, obj: AnyObject | any[], cachedPath?: string): WildCardSearchResult => {
    const pathArray = cachedPath ? cachedPath.split('.') : path.split('.');

    const search = (parsedPath: string[], obj: AnyObject | any[], results: any[], currentPath: string[], paths: string[]): void => {
