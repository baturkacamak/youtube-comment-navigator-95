...
class Logger {
  ...
  log(level: string, ...args: any[]): void {
    ...
    if (
      ...
      if (console && console.log) {
        console.log(
          `%c${timestamp} %c${emoji} [${this.prefix} ${level.toUpperCase()}]%c: ${mainMessage}`,
          'color: gray; font-style: italic;',
          style,
          'color: inherit;'
        );
  ...
  persistLogs(enable: boolean = true): void {
    this.persist = enable;
  }
  mockLogs(enable: boolean = true): void {
    this.mock = enable;
  }
  group(label: string): void {
    if (!this.mock) console.group(label);
  }
  groupEnd(): void {
    if (!this.mock) console.groupEnd();
  }
  ...
  private _logDetailedObject(level: 'warn' | 'error', data: any): void {
    ...
    const style = isError
      ? this.theme.error.replace('bold', 'normal')
      : this.theme.warn.replace('bold', 'normal');
    ...
    if (typeof data === 'object' && data !== null) {
      Object.entries(data).forEach(([key, value]) => {
        ...
        if (key.toLowerCase().includes('stack') && typeof value === 'string') {
          ...
          (key.toLowerCase().includes('reasons') || key.toLowerCase().includes('causes')) &&
          Array.isArray(value)
        ) {
